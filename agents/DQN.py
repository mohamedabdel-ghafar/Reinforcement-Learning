...
class DQNAgent(RLAgnet):
    def __init__(self, agent_name, create_nn,  state_dim, num_actions, mode: bool):
        ...
        with tf.name_scope(agent_name):
            ...
            if mode:
                ...
                self.q_s_a = tf.reduce_mean(tf.multiply(self.q, tf.one_hot(self.action, depth=num_actions)))
                self.train_op = self.optimizer.minimize(tf.square(self.y - self.q_s_a))
    ...
    def get_target_update_op(self):
        local_vars = tf.trainable_variables(scope=self.name + "/local")
        target_vars = tf.trainable_variables(scope=self.name + "/target")
        op = []
        for var, t_var in zip(local_vars, target_vars):
            op.append(tf.assign(t_var, var))
        return op
